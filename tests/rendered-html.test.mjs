...
const chapterSourceFiles = {
  "chapter-1": "01-zamok-nad-oblakami", "chapter-2": "02-son-ob-opustevshey-derevne",
  "chapter-3": "03-gorod-i-neznakomets", "chapter-4": "04-mezhdu-kovrami",
  "chapter-5": "05-prints-kotoryy-byl-voronom", "chapter-6": "06-porog-bez-prikazov",
  "chapter-7": "07-otvet-svistka", "chapter-8": "08-zerkalo-pervogo-snega",
  "chapter-9": "09-golos-v-beloy-perchatke", "chapter-10": "10-voron-kotoryy-znal",
  "chapter-11": "11-sovet-kotoryy-skazal-net", "chapter-12": "12-golos-bez-prikaza",
  "chapter-13": "13-tihiy-chas", "chapter-14": "14-chto-skryli-stareyshiny",
  "chapter-15": "15-malchik-iz-za-morya", "chapter-16": "16-zamok-kotoryy-vernulsya",
  "chapter-17": "17-chetyre-hranitelya", "chapter-18": "18-krug-goda",
};

/** Everything the narrator is supposed to say for a chapter, as one normalised line. */
async function spokenSource(id) {
  const markdown = await readFile(new URL(`../content/continuations/${chapterSourceFiles[id]}.md`, import.meta.url), "utf8");
...
test("ships complete revision-addressed Silero narration when generated", async (context) => {
  ...
  for (const voice of catalog.voices) {
    ...
    // No magic total here: it changed with every edit to the prose and told us
    // nothing about whether the audio still matched it. Voices are compared to each
    // other instead, which is what actually catches a half-finished generation.
    segmentCounts.push([voice.id, segmentCount]);
  }
  assert.ok(teoMarked > 0);
  for (const [voiceId, count] of segmentCounts) {
    assert.equal(count, segmentCounts[0][1], `${voiceId} has ${count} segments, ${segmentCounts[0][0]} has ${segmentCounts[0][1]}`);
  }
});

// The narrator speaks the manifest's own segment texts and never reconciles them
// with the page, so narration generated before a rewrite reads the previous draft
// aloud with nothing on screen to show it. Only a test can see that.
test("the shipped narration says what the chapters currently say", async (context) => {
  const catalogUrl = new URL("../public/audio/narration/catalog.json", import.meta.url);
  try {
    await access(catalogUrl);
  } catch {
  ...
  for (const voice of catalog.voices) {
    for (const [id] of readableChapters) {
      const manifest = JSON.parse(await readFile(new URL(`../public/audio/narration/${voice.id}/${id}/manifest.json`, import.meta.url), "utf8"));
      const spoken = manifest.segments.map((segment) => segment.text).join(" ").replace(/\s+/g, " ").trim();
      const written = await spokenSource(id);
      if (spoken !== written) {
        let at = 0;
        while (at < spoken.length && spoken[at] === written[at]) at += 1;
        stale.push(`${voice.id}/${id} diverges at char ${at}: narrated "${spoken.slice(at, at + 60)}" vs written "${written.slice(at, at + 60)}"`);
      }
    }
  }
  assert.deepEqual(stale, [], `regenerate the narration — ${stale.length} chapter package(s) narrate outdated text:\n${stale.join("\n")}`);
});
